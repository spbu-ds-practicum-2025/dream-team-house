...
Stateful распределённый сервис на Python (FastAPI), управляющий документом. Обрабатывает запросы на получение текущей версии документа, принимает и автоматически применяет правки, реплицирует изменения между тремя географически распределёнными узлами, контролирует бюджет потраченных токенов.
