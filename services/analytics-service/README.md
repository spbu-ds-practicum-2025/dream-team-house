...
Stateless сервис на Python (FastAPI) для сбора телеметрии и агрегации метрик. Принимает события от Text Service (
применение правок, репликация, ошибки) и предоставляет агрегированную статистику для визуализации.
...
- **Средняя задержка репликации**:
  `SELECT AVG((metadata->>'latency_ms')::int) FROM events WHERE event_type = 'replication_success'`
