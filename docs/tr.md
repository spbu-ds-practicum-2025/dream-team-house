...
1. Пользователь открывает веб-интерфейс|приложение для ПК.
2. Заполняет форму: указывает тему документа и вводит начальный текст.
3. Нажимает кнопку "Начать работу".
4. Система подтверждает начало работы, и AI-агенты приступают к генерации правок.

### Сценарий 2: Просмотр прогресса пользователем

1. Пользователь открывает страницу просмотра документа.
2. Frontend|Приложение для ПК периодически запрашивает и отображает текущую версию документа. При обнаружении новой
   версии подсвечиваются изменения (diff).
3. Пользователь переходит на вкладку "История правок", где видит список всех внесённых изменений с возможностью
   пагинации.
4. Пользователь открывает вкладку "Чат агентов", где в реальном времени отображается лента сообщений от AI-агентов.
5. Пользователь переходит на вкладку "Аналитика" и видит дашборд с агрегированными метриками и интерактивными
   графиками (количество правок, использованных токенов, число активных агентов).

### Сценарий 3: Завершение работы из-за исчерпания бюджета

1. В процессе работы система достигает установленного лимита по токенам.
2. Работа AI-агентов автоматически останавливается.
3. Frontend отображает пользователю уведомление о завершении работы.
4. Пользователю предлагается просмотреть финальную версию документа.
...
### Сценарий 1: Инициализация работы (Backend)

1. Frontend отправляет `POST /api/document/init` с данными из формы (тема, начальный текст).
2. Text Service получает запрос, создаёт в базе данных первую версию документа.
3. Система автоматически запускает Docker-контейнеры с AI-агентами, передавая каждому роль и токен через переменные
   окружения.
...
1. Агент, запущенный с переменными окружения (роль, токен, URL сервисов, ключ ProxyAPI), запрашивает текущий документ:
   `GET /api/document/current`.
2. Агент смотрит текущие сообщения в чате: `GET /api/chat/messages?since=<last_timestamp>`.
3. Получив текст, агент отправляет его вместе со своей инструкцией через ProxyAPI в OpenAI API для генерации правки.
4. Получив предложение от LLM, агент отправляет его в Text Service: `POST /api/edits`.
5. Агент публикует информационное сообщение в Chat Service: `POST /api/chat/messages`.
6. Агент ожидает 1 секунду и повторяет цикл.
7. При получении ответа `429 Too Many Requests` (превышение бюджета) агент завершает свою работу.
...
sequenceDiagram
    ...
    participant DBChat as PostgreSQL (Chat)
    participant Proxy as ProxyAPI
    participant OpenAI as OpenAI API
    Note over Agent: Запуск с переменными окружения (роль, токен, URL сервисов, ключ ProxyAPI)

    loop Цикл (каждую 1 секунду)
        ...
        Agent ->> LB: GET /api/chat/messages?since=<last_timestamp>
        LB ->> Chat: Forward request
        Chat ->> DBChat: SELECT messages WHERE ts > <last_timestamp>
        DBChat -->> Chat: [messages]
        Chat -->> LB: 200 OK
        LB -->> Agent: Chat messages
        Agent ->> Proxy: POST https://api.proxyapi.ru/openai/v1/responses
        Proxy ->> OpenAI: Forward request (instruction + document + chat)

        alt Успешная генерация
            OpenAI -->> Proxy: Generated edit proposal
            Proxy -->> Agent: Generated edit proposal
            Agent ->> LB: POST /api/edits
            LB ->> Text: Forward request
            Text ->> DB: INSERT edit proposal
            DB -->> Text: Saved
            Text -->> LB: 201 Created
            LB -->> Agent: Edit saved
            Agent ->> LB: POST /api/chat/messages
            LB ->> Chat: Forward request
            Chat ->> DBChat: INSERT informational message
            DBChat -->> Chat: Saved
            Chat -->> LB: 201 Created
            LB -->> Agent: Chat message published
            Agent ->> Agent: wait(1s)
        else 429 Too Many Requests (превышение бюджета)
            OpenAI -->> Proxy: 429 Too Many Requests
            Proxy -->> Agent: 429 Too Many Requests
            break Завершение цикла при 429
                Note over Agent: Остановить работу агента
            end
        end
    end
```
...
2. Сервис валидирует запрос (токен, корректность JSON, длина текста).
3. В рамках транзакции PostgreSQL проверяется бюджет токенов. Если он превышен, транзакция откатывается, и возвращается
   статус `429`.
4. Создаётся запись о правке со статусом `pending`.
5. Проверяется отсутствие конфликтов, после чего правка применяется к тексту, формируя новую версию документа.
6. Сохраняется новая версия, статус правки обновляется на `accepted`, счётчик токенов обновляется.
7. Транзакция коммитится.
8. Асинхронно отправляются:
    * Репликационное сообщение другим узлам: `POST /api/replication/sync`.
    * Событие в Analytics Service: `POST /api/analytics/events`.
9. Агенту возвращается ответ с ID правки, статусом и версией документа.
...
1. Узел Text Service (B) получает репликационное сообщение от узла (A) с новой версией документа.
2. После валидации источника узел B в транзакции проверяет, что версии последовательны, и применяет изменение.
3. Если версии не последовательны, узел B запрашивает пропущенные версии через `GET /api/replication/catch-up`.
4. Транзакция коммитится, и узел A получает подтверждение.
...
1. Docker-контейнер узла Text Service B падает. Load Balancer обнаруживает это через health check и исключает узел из
   пула маршрутизации.
2. Запросы обрабатываются оставшимися узлами A и C, которые продолжают обновлять версии документа.
3. Администратор перезапускает контейнер узла B.
4. При запуске узел B обнаруживает отставание в версиях (например, локальная версия 42, а актуальная 50).
5. Узел B отправляет запрос `GET /api/replication/catch-up?since_version=42` к узлу A.
6. Получив массив пропущенных версий, узел B последовательно применяет их.
7. После синхронизации Load Balancer возвращает узел B в пул.
...
1. Когда счётчик токенов приближается к лимиту (например, 15,000,000), Text Service обрабатывает последние допустимые
   правки.
2. При поступлении следующей правки, которая превысит лимит, Text Service откатывает транзакцию и возвращает
...
1. Агент 1 отправляет сообщение о своей текущей задаче в Chat Service: `POST /api/chat/messages` с телом
   `"Working on section X"`.
2. Chat Service сохраняет сообщение в Redis (например, используя `XADD`).
3. Агент 2 периодически запрашивает новые сообщения: `GET /api/chat/messages?since=<timestamp>`.
4. Получив информацию о том, что Агент 1 уже работает над секцией X, Агент 2 выбирает другую задачу, избегая
   дублирования работы.
...
### Сценарий 8: Просмотр аналитики (Backend)

1. Frontend отправляет запрос `GET /api/analytics/metrics?period=1h` в Analytics Service.
2. Analytics Service выполняет агрегирующие запросы к своей базе данных для подсчёта правок, токенов и активных агентов.
3. Сервис также выполняет запрос для получения временных рядов (например, использование токенов по минутам).
4. Собранные данные возвращаются на Frontend|Приложение для ПК для рендеринга дашборда.
...
- ✅ Контроль бюджета токенов функционирует корректно (сработает лимит на количество запросов).
- ✅ Покрытие тестами ≥ 60%.
